...
class CountMatrixLowRankLearning:
    def __init__(
        self,
        env,
        discretizer, 
        ...
        self.count_matrix = np.zeros(list(self.discretizer.n_states) + list(self.discretizer.n_actions))

        # Second try
        self.td_error_matrix = np.zeros_like(self.count_matrix)
        self.td_update_period = 5 # TD error更新周期
    ...
    # Second Try: introducing normalized TD error and N matrix into UCB
    def get_ucb_action(self, state):
        state_idx = self.discretizer.get_state_index(state)
        
        # 获取Q值
        q_values = self.L[state_idx + (slice(None),)].reshape(1, -1) @ self.R
        
        # 计算总访问次数
        total_visits = np.sum(self.count_matrix[state_idx]) + 1
        
        # 计算基于访问次数的bonus
        bonus_count = np.sqrt(np.log(total_visits) / (self.count_matrix[state_idx] + 1))
        
        # 计算基于TD error的bonus
        bonus_error = np.abs(self.td_error_matrix[state_idx])
        
        # 归一化
        delta = 1e-8  # 避免除零的小常数
        normalized_count = bonus_count / (np.max(bonus_count) + delta)
        normalized_error = bonus_error / (np.max(bonus_error) + delta)
        
        # 计算最终的UCB值
        ucb_values = q_values + self.c * (normalized_count + normalized_error)
        
        action_idx = np.unravel_index(np.argmax(ucb_values), self.discretizer.n_actions)
        return self.discretizer.get_action_from_index(action_idx)
    
    # UCB before second try
    # def get_ucb_action(self, state):
    #     state_idx = self.discretizer.get_state_index(state)
    ...
    #     # Calculate total visits for this state
    #     total_visits = np.sum(self.count_matrix[state_idx]) + 1
        
    #     # Calculate UCB scores
    #     ucb_values = q_values + self.c * np.sqrt(
    #         np.log(total_visits) / (self.count_matrix[state_idx] + 1)
    #     )
        
    #     # Get action with highest UCB score
    #     action_idx = np.unravel_index(np.argmax(ucb_values), self.discretizer.n_actions)
    #     return self.discretizer.get_action_from_index(action_idx)

    def choose_action(self, state):
        if np.random.rand() < self.epsilon:
            return self.get_ucb_action(state)
    ...
    # Third try: Try to updata TD error for every 10 steps
    # Fourth try: Try to updata TD error for every 5 steps
    def update_q_matrix(self, state, action, state_prime, reward, done):
        ...
        # 更新访问计数
        self.count_matrix[state_idx + tuple(action_idx)] += 1

        # 计算当前error
        q_next = np.max(self.L[state_prime_idx + (slice(None),)].reshape(1, -1) @ self.R) if not done else 0
        target_q = reward + self.gamma * q_next
        q = np.dot(self.L[state_idx + (slice(None),)], self.R[(slice(None),) + tuple(action_idx)])
        error = target_q - q

        #3. 每10步更新一次TD error矩阵
        #4. 每5步更新一次TD error矩阵
        self.step_counter += 1
        if self.step_counter % self.td_update_period == 0:
            self.td_error_matrix[state_idx + tuple(action_idx)] = error

        grad_R = self.R[(slice(None),) + tuple(action_idx)]
        grad_L = self.L[state_idx + (slice(None),)]
        
        self.L[state_idx + (slice(None),)] += self.alpha * error * grad_R / np.linalg.norm(grad_R)
        self.R[(slice(None),) + tuple(action_idx)] += self.alpha * error * grad_L / np.linalg.norm(grad_L)
    ...
    #     # First try: WeightCountMLR
    #     # visit_weight = np.log(self.count_matrix[state_idx + tuple(action_idx)] + 1) 
    #     # self.L[state_idx + (slice(None),)] += self.alpha * visit_weight * error_signal * grad_R / np.linalg.norm(grad_R)
    #     # self.R[(slice(None),) + tuple(action_idx)] += self.alpha * visit_weight * error_signal * grad_L / np.linalg.norm(grad_L)

    #     self.L[state_idx + (slice(None),)] += self.alpha * error_signal * grad_R / np.linalg.norm(grad_R)
    #     self.R[(slice(None),) + tuple(action_idx)] += self.alpha * error_signal * grad_L / np.linalg.norm(grad_L)
    ...
    def run_episode(self, is_train=True, is_greedy=False):
        ...
        for step in range(self.max_steps):
            ...
            if isinstance(info, dict) and 'terminal_observation' in info:  # Handle new gym API
