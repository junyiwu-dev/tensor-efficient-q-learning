...
class TensorLowRankLearning:
    def __init__(
        ...
        episodes,
        max_steps,
        epsilon,          # 用于 Epsilon-Greedy 的初始值
        alpha,
        gamma,
        k,
        decay=1.0,        # Epsilon 衰减率
        decay_alpha=1.0,
        init_ord=1,
        min_epsilon=0.0,  # Epsilon 最小值
        bias=0.0,
        normalize_columns=False,
        convergence_threshold=0.01,
        max_inner_iterations=5,
        ucb_c=1.0,        # UCB 探索系数
        lambda_penalty=0.01,  # 惩罚项系数
        epsilon_penalty=1e-6  # 惩罚项分母中的小常数
    ):
        self.env = env
        ...
        self.N = np.zeros(q_error_shape, dtype=np.float64)  # 访问计数矩阵
        self.N_total = np.zeros(tuple(self.discretizer.n_states), dtype=np.float64)  # 状态总访问次数
    ...
    def update_q_matrix(self, state, action, state_prime, reward, done):
        ...
        while iteration < self.max_inner_iterations:
            ...
            for factor_idx in self.factor_indices:
                ...
                # 基于频率的鼓励项 - 对低访问次数的状态-动作对给予更多关注
                # 由于目标函数中是减号，所以这里的梯度也需要取反
                encouragement_update = penalty_weight * grad_factor / np.linalg.norm(grad_factor)

                # 总更新 (error_update - encouragement_update)
