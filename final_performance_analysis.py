...
plt.rcParams.update({
    'font.size': 16,          # 12 → 16
    'axes.labelsize': 18,     # 14 → 18
    'axes.titlesize': 20,     # 16 → 20
    'legend.fontsize': 16,    # 新增
    'xtick.labelsize': 14,    # 新增
    'ytick.labelsize': 14,    # 新增
    'figure.figsize': (12, 6), # 调整为更紧凑：(14, 8) → (12, 6)
})
...
def plot_final_performance_comparison():
    ...
    for env_idx, env in enumerate(environments):
        ...
        # 对于cartpole环境设置特殊的y轴范围
        if env == 'cartpole':
            ax.set_ylim(80, None)  # 从50开始显示，避免数据过于集中
    ...
    # plt.savefig('final_performance_analysis_improved.png', dpi=300, bbox_inches='tight')
    plt.show()
