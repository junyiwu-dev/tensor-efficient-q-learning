...
plt.rcParams.update({
    ...
    'axes.prop_cycle': plt.cycler('color', ['#D2691E', '#009E73'])  # 橙色和绿色，用于 Without 和 With Penalty
})
...
def plot_comparison(ax, data_without_penalty, data_with_penalty, metric, title, env, smooth_window=20):
    ...
    只包含两条曲线：Without Penalty, With Penalty
    """
    ...
    # 绘制 Without Penalty TEQL - 使用橙色虚线（为色盲友好）
    if without_len > 0:
...
def main():
    base_dir = 'results_backup_TEQL_wo_penalty'  # 第一个结果文件夹
    penalty_dir = 'results_backup_TEQL_penalty'  # 第二个结果文件夹（带惩罚项）

    smooth_window = 50  # 平滑窗口大小

    # 读取 Without Penalty 数据
    ...
    # 创建绘图：2 行 2 列，添加整体标题
    fig, axes = plt.subplots(2, 2, figsize=(18, 14))  # 增大图形尺寸
